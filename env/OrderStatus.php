  ...
  private function doStatusRequest() {
    ...
    if($source !== false) {
	  ...
	  if(count($this->respErrorsList) == 0) { 
        $this->orderInfo = array("emcRef" => $this->xpath->evaluate("/order/emc_reference")->item(0)->nodeValue, "state" => $this->xpath->evaluate("/order/state")->item(0)->nodeValue, 
          "opeRef" => $this->xpath->evaluate("/order/carrier_reference")->item(0)->nodeValue, 
          "labelAvailable" => (bool)$this->xpath->evaluate("/order/label_available")->item(0)->nodeValue, "labelUrl" => $this->xpath->evaluate("/order/label_url")->item(0)->nodeValue
        );
      }
    }
  }
